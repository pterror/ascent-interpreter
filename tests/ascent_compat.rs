...
fn rel_set(engine: &mut Engine, name: &str) -> RelSet {
    RelSet(
        engine
            .relation(name)
            .unwrap()
...
fn test_fizzbuzz() {
    ...
    let fizz = rel_set(&mut engine,"fizz");
    assert_eq!(
        fizz,
        [3, 6, 9, 12].into_iter().map(|v| vec![i(v)]).collect()
    );

    let buzz = rel_set(&mut engine,"buzz");
    assert_eq!(buzz, [5, 10].into_iter().map(|v| vec![i(v)]).collect());

    let fizz_buzz = rel_set(&mut engine,"fizz_buzz");
    assert_eq!(fizz_buzz, [vec![i(15)]].into_iter().collect());

    let other = rel_set(&mut engine,"other");
    assert_eq!(
        other,
        [1, 2, 4, 7, 8, 11, 13, 14]
            .into_iter()
...
fn test_even_odd_negation() {
    ...
    let even = rel_set(&mut engine,"even");
    assert_eq!(even, [2, 4].into_iter().map(|v| vec![i(v)]).collect());

    let odd = rel_set(&mut engine,"odd");
...
fn test_arithmetic_join() {
    ...
    let bar = rel_set(&mut engine,"bar");
...
fn test_multi_head_rule() {
    ...
    let left = rel_set(&mut engine,"left");
    assert_eq!(left, [1, 2, 3].into_iter().map(|v| vec![i(v)]).collect());

    let right = rel_set(&mut engine,"right");
    assert_eq!(
        right,
...
fn test_self_loop_repeated_vars() {
    ...
    let self_loop = rel_set(&mut engine,"self_loop");
...
fn test_disjunction() {
    ...
    let eos = rel_set(&mut engine,"even_or_square");
    // Even: 2, 4, 6, 8, 10
...
fn test_disjunction_transitive() {
    ...
    let conn = rel_set(&mut engine,"connected");
...
fn test_negation_simple() {
    ...
    let baz = rel_set(&mut engine,"baz");
    assert_eq!(
        baz,
...
fn test_aggregation_grouped() {
    ...
    let team_max = rel_set(&mut engine,"team_max");
    assert_eq!(
        team_max,
...
fn test_generator_with_seed() {
    ...
    let number = rel_set(&mut engine,"number");
    assert_eq!(
        number,
        [0, 1, 2, 10, 11, 12]
            .into_iter()
...
fn test_multiple_conditions() {
    ...
    let special = rel_set(&mut engine,"special");
    assert_eq!(
        special,
...
fn test_three_way_join() {
    ...
    let result = rel_set(&mut engine,"result");
    assert_eq!(
        result,
...
fn test_mutual_recursion() {
    ...
    let is_even = rel_set(&mut engine,"is_even");
    let is_odd = rel_set(&mut engine,"is_odd");
...
fn test_empty_relations() {
    ...
    let negated = rel_set(&mut engine,"negated");
...
fn test_rule_chaining() {
    ...
    let result = rel_set(&mut engine,"final_result");
...
fn test_constant_in_clause() {
    ...
    let ones = rel_set(&mut engine,"ones");
...
fn test_if_let_option() {
    ...
    let output = rel_set(&mut engine,"output");
    assert_eq!(
        output,
