...
- [ ] **Cache `packed_data_ptr` for recursive rules (Phase 2)** — for recursive rules, `packed_data_ptr` is re-fetched per outer-scan tuple because `packed_try_insert` may reallocate the Vec. **Current benchmark (2026-03-16):** fibonacci jit_hot/20 = 11.9 µs vs ascent_macro 8.0 µs (1.49×); triangle jit_hot/20 = 245 µs vs ascent_macro 34 µs (7.2×). CLIF confirms fn2 (packed_data_ptr) is called once per outer loop iteration for recursive clauses. **Assessment:** savings < 0.5 µs at current scale. Triangle is unaffected (non-recursive). Defer — small savings.
