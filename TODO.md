...
- [ ] **Specialize interned ordering comparisons to skip callback** — currently `jit_cmp_interned` is called for every Lt/Le/Gt/Ge on interned columns (one vtable dispatch per comparison). If the intern table guarantees monotonic ID assignment matching semantic order (e.g. strings inserted in sorted order), raw u32 comparison would be correct and ~10× faster (no callback, no vtable dispatch). **Approach (needs careful design):** track a `monotonic: bool` flag on `InternTable` that starts true and flips to false if an out-of-order insertion is detected; the JIT checks this flag at compile time and emits inline `cmp` when true. **Risks:** (1) the flag must be checked at JIT compile time, not runtime — stale compiled code with inline `cmp` would silently produce wrong results if the flag flips after compilation; (2) cross-table comparisons must always use the callback; (3) the flag only helps workloads where strings arrive in sorted order (common in LSP symbol tables, uncommon in general). We probably want this but need to be VERY careful — wrong specialization = silent data corruption.
