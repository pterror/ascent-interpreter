...
5. [x] **asm write path** (~120 lines): replace `emit_heads`+`packed_try_insert` with direct stores to `head_jitrel.data`, inline `JitTupleSet` insert, `len++`, bounds-check → `call jit_rel_data_grow`. Two-phase dedup: probe `total.tuple_set` first (cross-iteration), then probe `new.tuple_set` (within-iteration). `JitNativeRelData{total, recent, new}` on `PackedStorage`; `advance_jit` flushes `new` buffer, rebuilds `jit_native`. **Pre-advance optimization:** eager pre-advance in `build_stratum_stage4_native_runtime` (calls `advance_jit()` once, moving EDB delta→recent); subsequent upfront JIT advances see empty delta and take cheap EDB path (no double-build). **Benchmarks (2026-03-16, n=20):** `jit_hot` 276 µs (6.7× vs macro 41 µs); `jit_run_only` 339 µs. Major improvements vs pre-Step-5 baseline: n=20 jit_hot -34%, n=30 jit_hot -13%.
