...
**Enable IDB inner clauses in asm native path (2026-03-16):** ✅ DONE. Removed stale rejection guard in `emit_clause_level`: `is_contiguous` was forced `false` when `is_rec=true`, causing the native path to reject any stratum with an IDB inner clause. Guard was originally written for the old `JitHashIndex` (linked-list) backend; native path always uses `JitColIndex` (contiguous) regardless of EDB/IDB. Fix: `is_contiguous = p.use_jit_native || !is_rec`. Correctness verified: `tc_shared_jit` tests pass. **Benchmark results (2026-03-16, n=50/20 jit_hot):** TC asm native: ~2.7ms (vs Cranelift fallback ~2.4ms, macro ~150µs). Fibonacci asm native: ~51µs (vs Cranelift fallback ~17µs, macro ~10µs). Both show regression vs Cranelift fallback — root cause is `extend_and_rebuild_indices` per-iteration cost for IDB total rebuild, same issue as triangle. Native path is now correct for IDB inner clauses; performance improvement requires incremental IDB index updates (see step 5 blocker note).
