...
- [ ] **JitColIndex binary-search existence check** — `emit_native_edb_full_probe` implements linear scan (O(n)). Replace with binary search over the sorted vals slice: log₂(99) ≈ 7 iterations × 5 instructions ≈ 35 instructions per probe, competitive with JitTupleSet's 43. Prerequisites: vals must remain sorted (currently true). Risk: binary search has poor branch prediction (7 unpredictable branches vs JitTupleSet's ~2). Only beneficial if JitTupleSet genuinely has L3 misses, which does not happen for the hot triangle benchmark where the table fits in L2.
