...
- [x] **Dedup outer full-scan on `meaningful_fresh_cols` (2026-03-17)** — added `meaningful_fresh_cols: Vec<(usize, VarId)>` to `CClause`, computed in Phase 4 of `optimize_body` via a backwards pass (heads ∪ body[i+1..] var union). In `stream_clause`, when `bound_cols` is empty and `meaningful_fresh_cols` is a proper subset of `fresh_cols` (wildcard detection) and conditions are empty, iterate only distinct values of the meaningful cols instead of all tuples. Single-meaningful-col fast path uses `FxHashSet<Value>` directly (no Vec). For CC `reach(x, _0)`, reduces stream_aggregation calls from ~380 to 20 at n=20. Benchmark pending re-measurement on quiet system (system load 11.5/16 CPUs at commit time).
