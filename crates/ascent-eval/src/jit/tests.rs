...
/// Run a program with JIT and without, assert same results. Returns the result count.
fn assert_jit_equivalence(input: &str, facts: &[(&str, Vec<Vec<Value>>)], query_rel: &str) -> usize {
...
// ─── Adversarial parity tests: RelIndex (IDB body) & inline dedup (sink heads) ───
...
#[cfg(feature = "specialized")]
#[test]
fn test_relindex_idb_empty_relation() {
    // TC with zero edges — RelIndex probe on empty IDB relation.
    let count = assert_jit_equivalence(TC_PROGRAM, &[], "path");
    assert_eq!(count, 0, "path should be empty with no edges");
}

#[cfg(feature = "specialized")]
#[test]
fn test_relindex_idb_chain() {
    // TC with chain 1→2→3→4→5→6 — verifies RelIndex across multiple fixpoint iterations.
    let edges: Vec<Vec<Value>> = (1..=5)
        .map(|i| vec![Value::I32(i), Value::I32(i + 1)])
        .collect();
    let count = assert_jit_equivalence(TC_PROGRAM, &[("edge", edges)], "path");
    // Chain of 5 edges over 6 nodes: C(6,2) = 15 transitive pairs.
    assert_eq!(count, 15, "chain of 5 edges should produce 15 path pairs");
}

#[cfg(feature = "specialized")]
#[test]
fn test_relindex_idb_growth_large() {
    // TC with 30-edge chain — triggers RelIndex table growth multiple times (>12 distinct keys).
    let edges: Vec<Vec<Value>> = (1..=30)
        .map(|i| vec![Value::I32(i), Value::I32(i + 1)])
        .collect();
    let count = assert_jit_equivalence(TC_PROGRAM, &[("edge", edges)], "path");
    // Chain of 30 edges over 31 nodes: C(31,2) = 465 transitive pairs.
    assert_eq!(
        count, 465,
        "chain of 30 edges should produce 465 path pairs"
    );
}

#[cfg(feature = "specialized")]
#[test]
fn test_relindex_idb_per_key_growth() {
    // Star topology: hub node 1 connected to 24 leaves (2..=25).
    // Tests per-key vals growth (>4 initial cap) in RelIndex.
    let edges: Vec<Vec<Value>> = (2..=25)
        .map(|i| vec![Value::I32(1), Value::I32(i)])
        .collect();
    let count = assert_jit_equivalence(TC_PROGRAM, &[("edge", edges)], "path");
    // Star: no transitive closure beyond direct edges (leaves don't connect).
    assert_eq!(
        count, 24,
        "star with 24 leaves should produce exactly 24 path pairs"
    );
}

#[cfg(feature = "specialized")]
#[test]
fn test_inline_dedup_sink_triangle() {
    // Complete graph K_15 — triggers jit_dedup_grow_relink during inline insertion.
    let n = 15i32;
    let edges: Vec<Vec<Value>> = (0..n)
        .flat_map(|i| ((i + 1)..n).map(move |j| vec![Value::I32(i), Value::I32(j)]))
        .collect();
    let count = assert_jit_equivalence(
        r#"
            relation edge(i32, i32);
            relation triangle(i32, i32, i32);
    ...
    // C(15,3) = 455 ordered triangles.
    assert_eq!(count, 455, "K_15 should produce C(15,3) = 455 triangles");
}

#[cfg(feature = "specialized")]
#[test]
fn test_inline_dedup_high_duplicates() {
    // Rule producing many duplicates — tests dedup filtering efficiency.
    let mut input_facts: Vec<Vec<Value>> = Vec::new();
    for x in 1..=3 {
        for i in 1..=50 {
            input_facts.push(vec![Value::I32(x), Value::I32(i)]);
        }
    }
    let count = assert_jit_equivalence(
        r#"
    ...
    assert_eq!(
        count, 3,
        "150 input tuples with 3 distinct first-columns should yield 3 output tuples"
    );
}

#[cfg(feature = "specialized")]
#[test]
fn test_inline_dedup_empty_output() {
    // Rule that filters everything — tests zero insertions into dedup table.
    let input_facts: Vec<Vec<Value>> = (1..=5).map(|i| vec![Value::I32(i)]).collect();
    let count = assert_jit_equivalence(
        r#"
            relation input(i32);
            relation output(i32);
            output(x) <-- input(x), if x < 0;
        "#,
        &[("input", input_facts)],
        "output",
    );
    assert_eq!(count, 0, "no input satisfies x < 0, output should be empty");
}

#[cfg(feature = "specialized")]
#[test]
fn test_relindex_idb_self_join() {
    // IDB relation used as both outer scan and inner probe in the same rule.
    // Tests RelIndex updated AND probed for the same relation across iterations.
    // Uses reach(x,y),base(y,z) ordering (IDB first, EDB second) to exercise
    // the IDB RelIndex probe path without hitting the known self-join SIGSEGV.
    let count = assert_jit_equivalence(
        r#"
            relation base(i32, i32);
            relation reach(i32, i32);
            reach(x, y) <-- base(x, y);
            reach(x, z) <-- reach(x, y), base(y, z);
        "#,
        &[(
            "base",
            vec![
    ...
    // Chain 1→2→3→4: all 6 transitive pairs.
    assert_eq!(count, 6, "IDB-first TC on 3 edges should produce 6 pairs");
}

#[cfg(feature = "specialized")]
#[test]
fn test_inline_dedup_multi_head_stratum() {
    // Multiple head relations in same stratum, both sinks.
    let program = r#"
        relation edge(i32, i32);
        relation forward(i32, i32);
        relation backward(i32, i32);
        forward(x, y) <-- edge(x, y);
        backward(y, x) <-- edge(x, y);
    "#;
    let facts: &[(&str, Vec<Vec<Value>>)] = &[(
        "edge",
        vec![
            vec![Value::I32(1), Value::I32(2)],
            vec![Value::I32(2), Value::I32(3)],
    ...
    let fwd_count = assert_jit_equivalence(program, facts, "forward");
    assert_eq!(fwd_count, 3, "forward should have 3 tuples");
