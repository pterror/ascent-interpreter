...
///   offset 64: jit_rel_specs   *const JitRelSpec              (8 bytes)
...
///   offset 72: jit_rel_ptrs    *mut *const JitRelData         (8 bytes)
...
///   offset 80: total_jit_rels u32                             (4 bytes)
///   offset 84: _pad5          u32                             (4 bytes)
#[repr(C)]
...
const _: () = {
    ...
    assert!(std::mem::offset_of!(StratumStage4Ctx, jit_rel_specs) == 64);
    assert!(std::mem::offset_of!(StratumStage4Ctx, jit_rel_ptrs) == 72);
    assert!(std::mem::offset_of!(StratumStage4Ctx, total_jit_rels) == 80);
};
