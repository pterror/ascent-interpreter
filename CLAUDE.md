...
Current state (2026-03-19): fibonacci jit_hot/20 ≈ 16µs vs ascent_macro ≈ 8µs (**~2×**); triangle jit_hot/20 ≈ 92µs vs ascent_macro ≈ 37µs (**~2.5×**); TC/50 ≈ 152µs vs ascent_macro ≈ 108µs (**~1.4×**). **Benchmark environment has high variance** (triangle jit_hot/20 fluctuates 80–180µs between runs on this system; always compare hot vs macro in the same run and check ratios, not absolutes). Key milestone: build_indices = jit_is_edb && !jit_is_sink (commit after 3bc173a) eliminates O(n log n) JitColIndex sort for IDB relations — only EDB inner-scan relations actually use JitColIndex in the native asm path; IDB inner clauses are rejected by the asm codegen. Saved ~10-13µs/iteration for triangle and similar savings for TC.
