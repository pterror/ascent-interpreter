...
Current state (2026-03-22): **The remaining gap is data structures, not code quality.** Disassembly of the macro's triangle inner loop shows `FxHash(imul 0xf1357aea2e62a9c5)` + hashbrown Swiss table probe (`pcmpeqb`/`pmovmskb`). The JIT emits equivalent hash probes. The gap is that the macro uses ONE hash structure (FxHashMap) for index + dedup + output, while the JIT uses THREE separate structures (JitColIndex sorted arrays + JitTupleSet + JitDedupTable) plus a batch flush cycle. Perf at n=50: JIT has 3.5× more instructions/iter (8.43M vs 2.43M). Perf breakdown at n=200: 69% in JIT scan code, 16% JitDedupTable::insert, 3% advance_jit_inner batch flush. The output pipeline (dedup + flush = 19%) is pure overhead not present in the macro. The scan code itself has ~3× more instructions because the batch flush→JitColIndex rebuild cycle forces sorted-array bulk reconstruction, separate dedup probes, and PackedStorage indirection. Fix: `RelIndex` (FxHash key→Vec<u32>, incremental insert) replaces JitColIndex for IDB; inline JitTupleSet insert eliminates separate JitDedupTable + batch flush. See TODO.md "Data-structure parity with ascent_macro" for full plan.

Triangle jit_hot/20 ≈ 106–114µs vs ascent_macro ≈ 50–57µs (**~2.1×** total). Eval-only ~1.2–1.3× at n=20. At n=50: 3.3× total. At n=200: 2.5× total. Large-n performance (n=200): jit_hot ~276ms vs ascent_macro ~109ms. Merge intersection (commit `b679163`): neutral-or-better for all n; 1.23× eval-only speedup at n=200 (JitTupleSet 393KB spills to L3, merge walks L1-cached slices). Keep `JitColIndex` for EDB (sorted vals needed for merge-scan). **Benchmark environment has high variance** — always compare hot vs macro in the same run and check ratios, not absolutes.
