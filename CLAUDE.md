...
Current state (2026-03-19): fibonacci jit_hot/20 ≈ 12µs vs ascent_macro ≈ 9µs (**~1.3×**); triangle jit_hot/20 fluctuates 100–160µs vs ascent_macro ≈ 37µs (**~2.8×**, high variance); TC/50 ≈ 152µs vs ascent_macro ≈ 110µs (**~1.38×**). Fibonacci gap closed from ~2× to ~1.3× by: (1) swap+clear instead of take in advance_jit_inner, (2) JitHashIndex::reserve pre-sizes hash index before first insert, (3) StratumStage4Runtime pool eliminates ~9 Box allocs per hot-bench iteration by reusing allocated runtime across engine instances (repopulate_runtime writes new engine-specific pointers without allocating; only applies to non-native-path strata). CRITICAL: pre-size jit_indices BEFORE build_stratum_stage4_runtime — handles capture entries_ptr at build time; realloc after capture → dangling ptr → SIGSEGV on first JIT probe. **Benchmark environment has high variance** (triangle jit_hot/20 fluctuates 100–180µs between runs on this system; always compare hot vs macro in the same run and check ratios, not absolutes). Current profile (post-8337ea5): `extend_and_rebuild_indices` is dominant for triangle (~7% samples) — JitTupleSet insertion for triangle.total (1140 tuples per fixpoint iteration); JIT inner loop (~9% samples ≈ 18µs).
