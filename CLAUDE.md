...
Current state (2026-03-17): fibonacci jit_hot/20 ≈ 16µs vs ascent_macro ≈ 9µs (**~1.8×**); triangle eval n=20 ≈ 70µs vs ascent_macro ≈ 34µs (**~2.1×** on clean system, high benchmark variance); TC approximately at parity. Phases 2–4 + pointer-increment loop (commits 5fa6362/8f947ab/9bea914/f43db74) reduced triangle n=20 eval from 133µs → ~70µs. **Benchmark environment has high variance** (ascent_macro triangle n=20 fluctuates 34–47µs between runs due to CPU cache/frequency effects; always compare hot vs macro in the same run). Profile from Phase 2 era (now stale): `stratum_stage4_0` JIT eval (~34%), `JitDedupTable::insert_if_new` (~15%), `insert_packed_raw` (~6%), malloc/cfree (~7%).

**Cross-block SSA in Cranelift increases register pressure.** When referencing an SSA value (CValue) from a predecessor block in a successor block, Cranelift keeps the value live across the edge. If the predecessor block already has many live values (~6), adding 2 more live values causes spilling that costs more than the instructions eliminated. Observed: dedup probe CSE (reuse entry_ptr/entry_hash from probe_loop in probe_check_found/probe_verify) regressed TC 57% despite saving ~5 instructions/probe. Rule: only move computations OUT of a block if the register pressure budget is clear.
