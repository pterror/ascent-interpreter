...
Current state (2026-03-19): fibonacci jit_hot/20 ≈ 12µs vs ascent_macro ≈ 8.5µs (**~1.4×**); triangle jit_hot/20 fluctuates 100–160µs vs ascent_macro ≈ 37µs (**~2.8–4.3×**, high variance); TC/50 ≈ 106µs (**~1.2×** vs ascent_macro ≈ 90µs). Fibonacci gap closed from ~2× to ~1.4× by: (1) swap+clear instead of take in advance_jit_inner (eliminates per-iteration malloc/free for delta/recent Vecs), (2) JitHashIndex::reserve pre-sizes hash index before first insert (eliminates 5–6 rehashes for n=20, 21 tuples). CRITICAL: pre-size jit_indices BEFORE build_stratum_stage4_runtime — handles capture entries_ptr at build time; realloc after capture → dangling ptr → SIGSEGV on first JIT probe. Pre-size jit_recent_indices is safe AFTER build because recent handles are only accessed after advance() refreshes them. **Benchmark environment has high variance** (triangle jit_hot/20 fluctuates 100–180µs between runs on this system due to CPU cache/frequency effects; always compare hot vs macro in the same run and check ratios, not absolutes). Current profile (post-8337ea5): `extend_and_rebuild_indices` is dominant (~7% samples) — JitTupleSet insertion for triangle.total (1140 tuples per fixpoint iteration); JIT inner loop (~9% samples ≈ 18µs).
