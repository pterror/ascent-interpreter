...
Current state (2026-03-18): fibonacci jit_hot/20 ≈ 19µs vs ascent_macro ≈ 10µs (**~1.9×**); triangle jit_hot/20 fluctuates 120–207µs vs ascent_macro ≈ 34µs (high variance, see below); TC approximately at parity. Recent asm backend optimizations: pointer-increment scan (commit `70caf94`), NativeCtx array-ptr cache slots (commit `180bd88`) — saves ~12 cycles/inner iter on triangle by caching `total_rels`/`head_rels`/`head_total_rels` pointers from NativeCtx into stack slots before each rule variant. **Benchmark environment has high variance** (triangle jit_hot/20 fluctuates 120–207µs between runs on this system due to CPU cache/frequency effects; always compare hot vs macro in the same run and check ratios, not absolutes). Profile from Phase 2 era (now stale): `stratum_stage4_0` JIT eval (~34%), `JitDedupTable::insert_if_new` (~15%), `insert_packed_raw` (~6%), malloc/cfree (~7%).
