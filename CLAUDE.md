...
Current state (2026-03-20): triangle jit_hot/20 ≈ 120–150µs vs ascent_macro ≈ 38–45µs (**~3–4×**; high variance, 80–180µs range). Per-iteration perf stat (same binary, total counts over criterion run): jit_hot 2.92M instructions / 1.51M cycles / 6,487 LLC-misses; ascent_macro 1.03M instructions / 583K cycles / 64 LLC-misses. Gap is **~2.8× instruction count** — structural, not bookkeeping. **Benchmark environment has high variance** (triangle jit_hot/20 fluctuates 80–180µs between runs on this system; always compare hot vs macro in the same run and check ratios, not absolutes). Key bookkeeping optimizations (2026-03-19): batch native flush (bd9df6b) eliminates 1140 per-tuple call overhead; sequential-index fast path (6b5c380) skips gather Vec for contiguous recent indices; EDB recent rebuild skip (8b2c8f5) avoids 3 allocs+frees per EDB relation per iteration when recent is already empty. JitSwissTable SIMD probe (2026-03-20, commit `3392df1`): no hot-benchmark improvement — for n=20 (190 edges) both JitTupleSet and JitSwissTable fit in L1/L2 after warmup; LLC misses are not the hot-path bottleneck; SIMD probe has ~15 more setup instructions than scalar probe for the common case.
